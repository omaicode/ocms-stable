...
use App\Exceptions\ComposerLoaderException;
use App\Exceptions\ThemeNotActiveException;
...
class ThemesManager
{
     ...
     * Get current theme.
     *
     * @return null|\App\Supports\Theme
     */
    public function current(): ?Theme
    {
        return $this->themes
            ->filter(function ($theme, $key) {
                return $key === config('theme.currentTheme', config('theme.fallbackTheme', 'default/theme-swipe'));
            })->first();
    }
     ...
     *
     * @return \App\Supports\ThemesManager
     */
