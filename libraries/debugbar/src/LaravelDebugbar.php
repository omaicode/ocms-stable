...
namespace OCMS\Debugbar;

use OCMS\Debugbar\DataCollector\AuthCollector;
use OCMS\Debugbar\DataCollector\CacheCollector;
use OCMS\Debugbar\DataCollector\EventCollector;
use OCMS\Debugbar\DataCollector\FilesCollector;
use OCMS\Debugbar\DataCollector\GateCollector;
use OCMS\Debugbar\DataCollector\LaravelCollector;
use OCMS\Debugbar\DataCollector\LogsCollector;
use OCMS\Debugbar\DataCollector\ModelsCollector;
use OCMS\Debugbar\DataCollector\MultiAuthCollector;
use OCMS\Debugbar\DataCollector\QueryCollector;
use OCMS\Debugbar\DataCollector\SessionCollector;
use OCMS\Debugbar\DataCollector\RequestCollector;
use OCMS\Debugbar\DataCollector\ViewCollector;
use OCMS\Debugbar\Storage\SocketStorage;
use OCMS\Debugbar\Storage\FilesystemStorage;
...
use OCMS\Debugbar\DataCollector\PhpInfoCollector;
use DebugBar\DataCollector\RequestDataCollector;
use DebugBar\DataCollector\TimeDataCollector;
use OCMS\Debugbar\DataFormatter\QueryFormatter;
use OCMS\Debugbar\Support\Clockwork\ClockworkCollector;
...
class LaravelDebugbar extends DebugBar
{
    ...
    public function boot()
    {
        ...
        /** @var \OCMS\Debugbar\LaravelDebugbar $debugbar */
        $debugbar = $this;
        ...
        if (!$this->isLumen() && $this->shouldCollect('route')) {
            try {
                $this->addCollector($this->app->make('OCMS\Debugbar\DataCollector\RouteCollector'));
            } catch (\Exception $e) {
                $this->addThrowable(
                    new Exception(
        ...
        if ($this->shouldCollect('models', true)) {
            try {
                $modelsCollector = $this->app->make('OCMS\Debugbar\DataCollector\ModelsCollector');
        ...
        if ($this->shouldCollect('livewire', true) && $this->app->bound('livewire')) {
            try {
                $livewireCollector = $this->app->make('OCMS\Debugbar\DataCollector\LivewireCollector');
        ...
        if ($this->shouldCollect('gate', false)) {
            try {
                $gateCollector = $this->app->make('OCMS\Debugbar\DataCollector\GateCollector');
