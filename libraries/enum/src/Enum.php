...
use OCMS\Enum\Casts\EnumCast;
use Illuminate\Support\Facades\Lang;
use Illuminate\Support\Traits\Macroable;
use OCMS\Enum\Contracts\EnumContract;
use OCMS\Enum\Contracts\LocalizedEnum;
use Illuminate\Contracts\Support\Arrayable;
use Illuminate\Contracts\Database\Eloquent\Castable;
use OCMS\Enum\Exceptions\InvalidEnumKeyException;
use OCMS\Enum\Exceptions\InvalidEnumMemberException;
...
abstract class Enum implements EnumContract, Castable, Arrayable, JsonSerializable
{
     ...
     * @return void
     *
     * @throws \OCMS\Enum\Exceptions\InvalidEnumMemberException
     */
     ...
     * @return static
     *
     * @throws \OCMS\Enum\Exceptions\InvalidEnumKeyException
     */
    ...
    protected static function getLocalizedDescription($value): ?string
    {
        if (static::isLocalizable()) {
            $localizedStringKey = static::getLocalizationKey() . '.' . strtolower(static::getKey($value));
            $localizedStringValue = static::getLocalizationKey() . '.' . $value;
            
            if (Lang::has($localizedStringKey)) {
                return __($localizedStringKey);
            }
            
            if (Lang::has($localizedStringValue)) {
                return __($localizedStringValue);
            }
        }
