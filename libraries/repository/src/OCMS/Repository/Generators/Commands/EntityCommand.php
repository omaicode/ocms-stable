...
class EntityCommand extends Command
{
    ...
    public function fire()
    {
        ...
        if ($this->confirm('Would you like to create a Presenter? [y|N]')) {
            $this->call('repository:presenter', [
        ...
        if ($validator == 'yes') {
            $this->call('repository:validator', [
                'name'    => $this->argument('name'),
        ...
        if ($this->confirm('Would you like to create a Controller? [y|N]')) {
            ...
            $controller_command = ((float) app()->version() >= 5.5  ? 'repository:rest-controller' : 'repository:resource');
            $this->call($controller_command, $resource_args);
        }

        $this->call('repository:repository', [
            'name'        => $this->argument('name'),
